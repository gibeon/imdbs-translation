...
### 2.1 发展历史     
&emsp;&emsp;尽管当前内存数据库领域突显活跃，但实际上自八十年代中期开始，内存优化系统的研究和开发已经有了很长的历史。本节总结了早期工作，并将其与当今系统所采用的体系结构进行了对比。   
#### 2.1.1 早期（1984-1994）   
&emsp;&emsp;80年代初，关于内存数据库技术的研究论文开始出现。总体而言，大部分工作在假设多数或所有数据库都可放入内存的前提下，着眼于提高传统关系数据库系统的性能。    
&emsp;&emsp;1976年发布的IMS Fast Path[50]是最早针对内存驻留数据进行优化的已知系统之一。Fast Path要求用户将数据库指定为内存驻留。优化包括细粒度记录级锁定和提交时安装记录更新。威斯康星大学的早期工作[37]探索了在大多数或所有关系都适合放入主存时优化关系数据库系统的方法。这项工作探讨了许多领域，包括访问方法性能、连接技术，以及恢复优化，如快速提交、组提交、并行日志I/O和检查点技术。    
&emsp;&emsp;威斯康星大学的MM-DBMS项目[80，79，81]在内存数据库的各个领域进行了开创性的研究。MM-DBMS探索了使用指针进行直接记录访问、新的内存优化索引方法（T-Trees）、优化的两阶段锁和新的恢复技术。IBM的Starburst内存驻留存储引擎[82]就基于MM-DBMS中所提出的几种优化（如T-Trees、恢复技术）。后续工作[52]探索了StarBurst中的锁和锁存优化，提出使用单独的一个锁存来保护表、表的所有索引和相关的锁信息。该工作还提出对锁数据的直接寻址，而避免间接寻址。   
&emsp;&emsp;MARS系统[41,58]将系统划分为内存数据库处理器（DP）和恢复处理器（RP），其中RP将易失性更新延迟复制到稳定存储。MARS的恢复技术展示了避免在内存系统中进行undo日志记录的好处。系统M[134，135]是为研究内存环境中的恢复技术而构建的原型。它也探索了一种分区技术，使用消息服务器处理事务，使用日志服务器持久化更新。        
&emsp;&emsp;TPK [90]探索了多处理器内存环境中的事务处理。TPK以串行方式执行事务，并使用一组专用线程（输入，输出，执行，恢复）来处理事务。   
&emsp;&emsp;IBM[21，156]的Office by-example(OBE)探索了集成化办公系统中内存数据查询处理的性能与优化。OBE研究了几种以读为主数据的内存优化，包括依赖指针进行直接内存访问的排序倒排索引。    
&emsp;&emsp;PRISMA/DB[12，13]探索了分片数据库管理系统上的两阶段锁和两阶段提交，其中每个节点都将数据存储在内存中。   
